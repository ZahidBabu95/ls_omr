        ...
        <form action="{{ route('admin.pricing.update', $pricing) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" required value="{{ old('name', $pricing->name) }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                    @error('name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
                
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Billing Cycle</label>
                    <input type="text" name="billing_cycle" required value="{{ old('billing_cycle', $pricing->billing_cycle) }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price" required value="{{ old('price', $pricing->price) }}" class="w-full md:w-1/2 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
            ...
            <div class="pt-4 border-t border-slate-100">
                ...
                <div id="features-container" class="space-y-3">
                    @if($pricing->features_json && count($pricing->features_json) > 0)
                        @foreach($pricing->features_json as $feature)
                            <div class="flex gap-2 feature-row">
                                <input type="text" name="features[]" value="{{ $feature }}" class="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 focus:outline-none focus:border-blue-500">
